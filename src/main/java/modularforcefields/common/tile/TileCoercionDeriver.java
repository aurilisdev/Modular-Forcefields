...
public class TileCoercionDeriver extends TileFortronConnective {
    ...
    public TileCoercionDeriver(BlockPos pos, BlockState state) {
        ...
        addComponent(new ComponentElectrodynamic(this, true, true).voltage(MFFSConstants.COERCIONDERIVER_VOLTAGE).setInputDirections(BlockEntityUtils.MachineDirection.BOTTOM));
        ...
        addComponent(new ComponentContainerProvider("coercionderiver", this).createMenu((id, player) -> new ContainerCoercionDeriver(id, player, getComponent(IComponentType.Inventory), getCoordsArray())));
    ...
    protected void tickServer(ComponentTickable tickable) {
        ...
        fortron.setValue((int) (fortron.getValue() + electro.extractPower(TransferPack.joulesVoltage(Math.min(getTransfer(), fortronCapacity.getValue() - fortron.getValue()), electro.getVoltage()), false).getJoules()));
        fortron.setValue(fortron.getValue() - sendFortronTo(Math.min(fortron.getValue(), getTransfer()), getConnectionTest()));
    }
