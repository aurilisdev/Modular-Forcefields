...
public class ScreenCoercionDeriver extends GenericScreen<ContainerCoercionDeriver> {
	...
	addComponent(new ScreenComponentElectricInfo(-AbstractScreenComponentInfo.SIZE + 1, 2)
		.wattage(electro -> electro.getHolder() instanceof TileCoercionDeriver deriver
	...
	addComponent(new ScreenComponentMultiLabel(0, 0, matrixStack -> {
	    ...
	    matrixStack.drawString(font,
		    MFFSTextUtils.gui("fortrondevice.transfer", ChatFormatter
			    ...
			    .append(" / s")),
		    25, 65, 4210752, false);
	    matrixStack.drawString(font, MFFSTextUtils.gui("fortrondevice.linked", deriver.getConnections()), 25, 55,
		    4210752, false);
	    matrixStack.drawString(font,
		    MFFSTextUtils.gui("fortrondevice.usage",
			    ChatFormatter.getChatDisplayShort(deriver.getTransfer() * 20, DisplayUnits.WATT)),
		    25, 45, 4210752, false);
	    matrixStack.drawString(font, MFFSTextUtils.gui("fortrondevice.frequency", deriver.getFrequency()), 25, 35,
		    4210752, false);
	}));
	imageHeight += 40;
	inventoryLabelY += 40;
