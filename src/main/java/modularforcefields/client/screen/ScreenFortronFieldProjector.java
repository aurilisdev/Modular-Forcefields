...
public class ScreenFortronFieldProjector extends GenericScreen<ContainerFortronFieldProjector> {
	...
	addComponent(new ScreenComponentMultiLabel(0, 0, poseStack -> {
	    ...
	    font.draw(poseStack, MFFSTextUtils.gui("fortrondevice.linked", projector.getConnections()), 25, 115,
		    4210752);
	    ...
	    font.draw(poseStack, MFFSTextUtils.gui("fortrondevice.frequency", projector.getFrequency()), 25, 95,
		    4210752);

	    font.draw(poseStack, MFFSTextUtils.gui("fieldprojector.status", projector.getStatus().name()), 8, 130,
		    4210752);
	    ...
	    fill(poseStack, barX, barY, barX + barWidth, barY + barHeight, 0xFF202020);

	    int healthColor = healthPercent > 0.5 ? 0xFF55FF55 : healthPercent > 0.25 ? 0xFFFFFF55 : 0xFFFF5555;

	    fill(poseStack, barX, barY, barX + filledWidth, barY + barHeight, healthColor);

	    fill(poseStack, barX, barY, barX + barWidth, barY + 1, 0xFF555555);
	    fill(poseStack, barX, barY + barHeight - 1, barX + barWidth, barY + barHeight, 0xFF555555);
	    fill(poseStack, barX, barY, barX + 1, barY + barHeight, 0xFF555555);
	    fill(poseStack, barX + barWidth - 1, barY, barX + barWidth, barY + barHeight, 0xFF555555);

	    font.draw(poseStack, Component.literal(String.format("%.0f / %.0f HP", currentHealth, maxHealth)), 25, 156,
		    4210752);
	}));
	imageHeight += 93;
	inventoryLabelY += 93;
