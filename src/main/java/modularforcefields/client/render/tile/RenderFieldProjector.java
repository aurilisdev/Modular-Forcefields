...
public class RenderFieldProjector extends AbstractTileRenderer<TileFortronFieldProjector> {
	...
	if (tile.fortron.getValue() > 0) {
	    ...
	    BakedModel shape = Minecraft.getInstance().getModelManager()
		    .getModel(MFFSClientRegister.MODEL_FIELDFORTRON);
		...
		ibakedmodel = Minecraft.getInstance().getModelManager()
			.getModel(MFFSClientRegister.MODEL_PREVIEWHALFSPHERE);
		break;
	    case PYRAMID:
		ibakedmodel = Minecraft.getInstance().getModelManager()
			.getModel(MFFSClientRegister.MODEL_PREVIEWPYRAMID);
		break;
	    case SPHERE:
		ibakedmodel = Minecraft.getInstance().getModelManager()
			.getModel(MFFSClientRegister.MODEL_PREVIEWSPHERE);
		break;
	    case CUBE:
